...
const newPeople = people.map(item => item.first_name)
newPeople
...
const newTheNumbers = theNumbers.map((theNumber, index) => {
  return {
    value: theNumber,
    index: index
  }
...
const newStarter = starter.map((item, index, starter) => {
  const nextIndex = (index+1)%starter.length;
  return item*starter[nextIndex]; 
});
newStarter